...
from trigger import PhoneDetector
import ikony
...
class App(tk.Tk):
    ...
    def video_stream(self) -> None:
        ...
        if img is not None:
            if self.trigger_run:
                detector = PhoneDetector(model_path=self.model_path,confidence_threshold=0.7)
        ...
        # first, size the image way back down, then resize it to the size of the canvas
    ...
    def selectshotsfolder(self) -> None:
        ...
        if self.shots_path == "" or self.shots_path == () or not os.path.isdir(self.shots_path): # check if the folder is selected
