...
class App(ctk.CTk):
    ...
    # Округление переменной CTk DoubleVar
    ...
    def start_trigger(self):
        if not hasattr(self, 'model_path') or not self.model_path:
            self.selectmodelpath()
    
        if not hasattr(self, 'shots_path') or not self.shots_path:
            self.selectshotsfolder()
    
        if self.model_path and self.shots_path:
            print("Starting trigger...")  # Добавлено отладочное сообщение
            #self.tabview.set("Blank")  # Переход на пустое окно, где будет поток
    ...
    # Остановка оригинального потока
        print("Stopping video stream...")  # Добавлено отладочное сообщение
        #self.video_thread.join()
        #self.camera.stop()
        
         # Инициализация и запуск триггера
        try:
            print(f"Initializing PhoneDetector with model: {model_path}")  # Добавлено отладочное сообщение
            self.detector = PhoneDetector(model_path=model_path, camera=self.camera, update_ui_callback=self.update_video_frame)
            print("Starting PhoneDetector...")  # Добавлено отладочное сообщение
            self.detector.run()
        except Exception as e:
            print(f"Error in run_trigger: {e}")  # Вывод ошибки для отладки
    ...
    def update_video_frame(self, frame):
        """Обновление фрейма."""
        print("Updating video frame...")  # Добавлено отладочное сообщение
